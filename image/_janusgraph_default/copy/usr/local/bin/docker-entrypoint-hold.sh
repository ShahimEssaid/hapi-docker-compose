...
if [ "$1" == 'janusgraph' ]; then
  ...
  while IFS=' ' read -r envvar_key envvar_val; do
    ...
    # Shahim: had to change the %d flag to _ to work around docker compose .env variable name rules.
    elif [[ "${envvar_key}" =~ gremlinserver(_)?[.]{1}(.+) ]]; then
    #elif [[ "${envvar_key}" =~ gremlinserver(%d)?[.]{1}(.+) ]]; then
      # Check for edit mode %d after prefix
      if [[ ${BASH_REMATCH[1]} == "_" ]]; then edit_mode="d"; else edit_mode="w"; fi
  ...
  if [ "$2" == 'show-config' ]; then
    ...
    # Shahim: hard coding this to a custom file because of limitations with the docker compose env files approach.
    exec ${JANUS_HOME}/bin/janusgraph-server.sh "${JANUS_CONFIG_DIR}/janusgraph-server-custom.yaml"
